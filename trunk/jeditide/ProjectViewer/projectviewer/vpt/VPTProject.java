...
public class VPTProject extends VPTNode {
	...
	public Icon getIcon(boolean expanded) {
		if (icon == null && iconPath != null) {
			try {
				icon = GUIUtilities.loadIcon(iconPath);
			} catch (Exception e) {
				Log.log(Log.ERROR, this, e);
			}
		}
		return (icon != null) ? icon : projectIcon;
	} //}}}
	 ...
	 *
	 * @param	path	The path to the new icon (null for default).
	 *
	 * @since PV 3.0.1
	 */
	public void setIconPath(String path)
	{
		if (path != null && !MiscUtilities.isURL(path)) {
			path = "file:" + path;
		}
		iconPath = path;
		icon = null;
	 ...
	 *
	 * @return The custom icon path, or null if no custom icon is set.
	 *
	 * @since PV 3.0.1
	 */
