 ...
 */
public class SideKickBrowser extends JPanel implements DefaultFocusComponent
{
	...
	//{{{ SideKickBrowser constructor
	public SideKickBrowser(View view, boolean docked)
	{
...
//                configMenu = new PopupMenu("Parse on...");
	...
	protected void update()
	{
		...
		else
		{
			...
			if(SideKick.isFollowCaret() && data != null) {
                TreePath p = data.getTreePathForPosition(view.getTextArea().getCaretPosition());
				tree.expandTreeAt(p);
            }
		}
		updateSearchData();
	...
	public void updateFilter(boolean with_delay)
	{
		...
		if (searchField.getText().length() == 0) {
			...
			if (jEdit.getBooleanProperty(SideKick.FILTER_VISIBLE)) {
				tree.filter_visible_nodes(ftm, visible, root);
			} else {
				tree.expandAll(true);
			}
		}
