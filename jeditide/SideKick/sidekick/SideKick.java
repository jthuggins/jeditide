 ...
 * It happens to be  sidekick.enhanced.SourceBrowser. 
 *  
 */
...
//		autoParse();
	...
	public void handleEditPaneUpdate(EditPaneUpdate epu)
	{
		...
		{
			if (!isParseOnChange()) {
				SideKickBrowser browser = (SideKickBrowser) view.getDockableWindowManager().getDockable("sidekick");
				if (browser != null) browser.reloadParserCombo();
				return;
			}
			// check if this is the currently focused edit pane
...
//			this.editPane = null;
			SideKickBrowser browser = (SideKickBrowser) view.getDockableWindowManager().getDockable("sidekick-tree");
			if (browser == null) return;
			browser.removeParserPanel();
		}
	} //}}}
	...
	private void activateParser()
	{
		...
		SideKickBrowser browser = (SideKickBrowser) view.getDockableWindowManager().getDockable("sidekick-tree");
		if (browser == null) return;
		browser.reloadParserCombo();
		if (parser != null)
			browser.addParserPanel(parser);
		parse(true);
	} //}}}
