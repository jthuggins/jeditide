...
public abstract class Importer implements Runnable {
		...
		throws IOException
	{
		...
		path = PVActions.stripSeparator(path);
	...
	protected void importFiles(VPTNode dest,
		...
		throws IOException
	{
		importFiles(dest, filter, recurse, flatten, false);
	}
	 ...
	 * @param	flatten	Whether to import all files (even from
	 *					sub-directories) into the dest node.
	 * @param	addHidden	Whether to add hidden files.
	 *
	 * @since PV 3.0.1
	 */
	protected void importFiles(VPTNode dest,
		...
		throws IOException
	{
		...
		for (String url : children) {
			if (!flatten) {
				String parent = PVActions.stripSeparator(vfs.getParentOfPath(url));
														...
														String name)
	{
		...
		dlg.setAddHiddenFiles(ImportUtils.getAddHiddenFiles(project.getProperties(),
															name));
